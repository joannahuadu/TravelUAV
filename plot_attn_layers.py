...
def average_heads(t: torch.Tensor) -> np.ndarray:
    """
    t: Tensor shape (1, heads, N, N)
    returns: ndarray shape (N, N) averaged over heads
    """
    if t.ndim != 4:
        raise ValueError(f"Expected tensor of shape (1, H, N, N), got {tuple(t.shape)}")
    # average over head dim=1
    t_mean = t.mean(dim=1)  # (1, N, N)
    # remove batch dim
    t_mean = t_mean.squeeze(0)  # (N, N)
    return t_mean.to(dtype=torch.float32).cpu().numpy()
...
def plot_all_steps_grid(all_steps, layer_indices, step_nums, out_path,
                        dpi=300, img_start=80, img_len=85,
                        cmap="inferno"):
    ...
    layer_names = [f"Layer {layer_indices[0]}",
                   f"Layer {layer_indices[1]}",
                   f"Layer {layer_indices[2]}"]
...
def main():
    ...
    for i, fpath in enumerate(files):
        ...
        data = torch.load(fpath, map_location=args.device)
        num_layers = len(data)
        l_first, l_mid, l_last = select_layers(num_layers)

        averaged_layers = [average_heads(t) for t in data]

        all_steps.append([averaged_layers[l_first],
                        averaged_layers[l_mid],
                        averaged_layers[l_last]])
        step_nums.append(step_num)

    out_path = os.path.join(args.output_dir, "{}_attn.png".format(traj_id))
    plot_all_steps_grid(all_steps, (l_first, l_mid, l_last), step_nums, out_path)
