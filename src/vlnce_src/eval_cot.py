...
def eval(model_wrapper: BaseModelWrapper, assist: Assist, eval_env: AirVLNENV, eval_save_dir):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            inputs, rot_to_targets = model_wrapper.prepare_cot_inputs(batch_state.episodes, batch_state.target_positions)
            ...
            for t in range(int(args.maxWaypoints) + 1):
                ...
                inputs, _ = model_wrapper.prepare_cot_inputs(batch_state.episodes, batch_state.target_positions, assist_notices)
...
if __name__ == "__main__":
    ...
    eval_env = initialize_env_eval(dataset_path=dataset_path, save_path=eval_save_path, eval_json_path=eval_json_path, eval_split=args.eval_split)
