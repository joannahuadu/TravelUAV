...
# import torch
# a = torch.load("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/Model/LLaMA-UAV/work_dirs/llama-vid-7b-pretrain-224-uav-full-data-lora32_bs128/checkpoint-5000/non_lora_trainables.bin")
# b = torch.load("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/Model/LLaMA-UAV/work_dirs/llama-vid-7b-pretrain-224-uav-full-data-lora32_bs128/checkpoint-5000/mm_projector.bin")
# c = torch.load("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/Model/LLaMA-UAV/work_dirs/llama-vid-7b-pretrain-224-uav-full-data-lora32_bs128/checkpoint-5000/checkpoint-5000.bin")
# d = torch.load("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/Model/LLaMA-UAV/work_dirs/llama-vid-7b-pretrain-224-uav-full-data-lora32_bs128/non_lora_trainables.bin")
# e = torch.load("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/Model/LLaMA-UAV/work_dirs/llama-vid-7b-pretrain-224-uav-full-data-lora32_bs128/mm_projector.bin")
...
# # print(torch.sum(a["base_model.model.waypoint_emb.weight"] == c["base_model.model.waypoint_emb.weight"]))
# # print(a["base_model.model.waypoint_emb.weight"].shape.numel())
...
#                 result.append({"success": traj_dir.split("_")[1]})
#             elif 'oracle' in traj_dir:
#                 result.append({"oracle": traj_dir.split("_")[1]})
#             else:
#                 result.append({"fail": traj_dir})  # Assuming non-marked dirs are 'success'

#     return result

# def split_data_ori(path):
#     """Split the dataset into different categories based on the path type."""
#     dirs = os.listdir(path)

#     return [traj_dir.split("_")[1] if 'success' in traj_dir or 'oracle' in traj_dir else traj_dir for traj_dir in dirs if 'record' not in traj_dir and 'dino' not in traj_dir]

# analysis_path_1 = "/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV_data/TravelUAV/eval_closeloop_llavanext_bs128_all_fast"
# analysis_path_2 = "/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV_data/TravelUAV/eval_closeloop_llava_llama_attn_bs128_all_fast"
# analysis_dirs_1 = split_data(analysis_path_1)
# analysis_dirs_1_ori = split_data_ori(analysis_path_1)
# analysis_dirs_2 = split_data(analysis_path_2)
# # print(len(analysis_dirs_1))
# # print(len(analysis_dirs_2))
# total = len(analysis_dirs_1)
# success = 0
# oracle = 0
# success_dirs = []

# for traj_dir in analysis_dirs_1:
#     # print(traj_dir)
#     if 'success' in traj_dir:
#         success += 1
#         oracle += 1
...
# oracle = 0
# success_dirs = []
# for traj_dir in analysis_dirs_2:
#     if list(traj_dir.values())[0] in analysis_dirs_1_ori:
#         if 'success' in traj_dir:
#             success += 1
#             oracle += 1
#             success_dirs.append(traj_dir)
...
# print(f"SR: {sr:.2f}%, OSR: {osr:.2f}%")
# print(f"Successful trajectories: {total}")

# import json
# with open('/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV_data/TravelUAV/Carla_Town05/fe734e53-c9a5-4010-9f9d-ee6fdd4c0307/merged_data.json', 'r') as f:
#     data = json.load(f)
    
# print(data['trajectory_raw_detailed'])
...
# 假设文件名为 data.json
with open("/mnt/data1/workspace/wmq/TravelUAV_ws/TravelUAV/data/uav_dataset/trainset.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
for item in data:
    ...
    json_path = item["json"]
    # 拆分路径并去掉最后一个部分（文件名）
    split_path = "/".join(json_path.split("/")[:-1])
    # 或者用 os.path.dirname(json_path)
    paths.add(split_path)

print(f"共有 {len(paths)} 个不同的路径：")
# for p in paths:
#     print(p)
